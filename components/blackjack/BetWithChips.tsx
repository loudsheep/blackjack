    ...
    return (
        <>
            ...
            <div className='flex absolute w-[100vw] bottom-0 justify-center z-50'>
                <div className='flex flex-col h-36 w-1/2 bg-black bg-opacity-30 border-2 border-black rounded-lg text-white p-1'>
                    ...
                    <div className='flex flex-[2] justify-center'>
                        {availableChips.map((value) => (
                            <div className={'h-full aspect-square rounded-full flex justify-center items-center text-xl mx-1 ' + (value <= temporaryUserStack ? "bg-red-500 cursor-pointer" : "bg-gray-500 cursor-not-allowed")} onClick={() => handleChipAdd(value)}>
                                {value}
                            </div>
                        ))}
                    </div>
